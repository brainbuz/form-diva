...
Like Radio and Checkbox, Select takes a values argument. The values argument may be either a list to be used as both the value and the display value, or the values may the value the display text seperated by a colon.
...
To display different text than the values use a colon:
              [ 'usa:United States of America', 'china:Peoples Republic of China']          
