...
Returns a an array ref of the fields in order where the elements of the referenced array are each a hashref to any of these fields: name, type, extra, label, and if passed a hashref of data, value. This applies only to visible fields, if you want hidden fields you'll need to clone the object and make them visible.

There are two optional parameters: data and 'skipempty' which suppress returning fields with no data, when used without data it is necessary to pass an empty hashref.

  my $datavalues = $diva->datavalues( undef, 'skipempty');
  print $datavalues->[2]{value};
